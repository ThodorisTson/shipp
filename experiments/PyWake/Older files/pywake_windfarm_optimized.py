...
class WindFarmModel:
    ...
    def generate_timeseries(
        ...
        """
        Generate power production time series from wind data (OPTIMIZED TIME SERIES).
        
        This version uses PyWake's native time series simulation with automatic chunking.
        This is THE recommended approach from PyWake documentation for time series.
        Speed: Near-instantaneous for typical datasets!
        ...
        wd_chunks : int, optional
            Number of chunks to split the time series into (for memory management)
            If None, PyWake will process entire series at once
            Recommended: 4-10 for very large datasets (>10k hours)
            Default: None (no chunking, fastest)
        verbose : bool
        ...
        Notes
        -----
        This uses PyWake's native time series simulation which is MUCH faster than
        manual looping. The key is passing time=True to treat (wd, ws) as paired
        time series rather than a Cartesian product grid.
        """
        ...
        # Create site with time series support
        from py_wake.site.shear import PowerShear
        site = UniformSite(
            p_wd=[1],  # Single sector with uniform probability
            ti=ti,
        ...
        if verbose:
            print(f"Calculating power for {n_steps} timesteps (OPTIMIZED TIME SERIES)...")
            if wd_chunks:
                print(f"  Using {wd_chunks} chunks for memory management")
            else:
                print(f"  Processing entire time series in one call (fastest!)")
        
        # OPTIMAL APPROACH: Single PyWake call with entire time series!
        # The time=True parameter tells PyWake to treat (wd, ws) as paired time series
        # The wd_chunks parameter lets PyWake handle chunking internally if needed
        if wd_chunks is not None:
            sim_res = wake_model(
                self.x, self.y, 
                wd=wd_array, 
                ws=ws_array, 
                time=True,
                wd_chunks=wd_chunks  # PyWake handles chunking internally
            )
        else:
            sim_res = wake_model(
                self.x, self.y, 
                wd=wd_array, 
                ws=ws_array, 
                time=True  # No chunking - fastest for typical datasets
            )
        
        # Extract power and sum over turbines
        # Power is in W, convert to MW
        # The result has shape (n_turbines, n_timesteps) or similar
        power_da = sim_res.Power
        
        # Sum over turbine dimension to get farm total
        for dim in ("wt", "turbine", "iwt", "i"):
            if dim in power_da.dims:
                farm_power_w = power_da.sum(dim=dim).values
                break
        else:
            # Fallback: sum over first dimension if it's not time
            if len(power_da.dims) > 1:
                farm_power_w = power_da.sum(dim=power_da.dims[0]).values
            else:
                farm_power_w = power_da.values
        
        power_array = farm_power_w / 1e6  # Convert W to MW
        
        if verbose:
            print(f"  ✓ Complete! Mean power: {power_array.mean():.2f} MW")
            print(f"  Capacity factor: {power_array.mean() / (self.n_turbines * self.turbine_rating_mw) * 100:.1f}%")
