...
# PyWake imports
from pywake_windfarm_optimized import WindFarmModel, create_reference_offshore_windfarm, generate_synthetic_wind_data
...
# CHUNK_SIZE = 1500  # No longer needed with optimized time series approach
...
# OPTIMIZED: Single PyWake call for entire time series (no manual chunking!)
# For datasets > 10k hours with limited RAM, you can optionally add: wd_chunks=4
power_ts = wf.generate_timeseries(
    wind_data_subset['wind_speed'],
    wind_data_subset['wind_direction'],
    ti=0.06,
    wd_chunks=None,  # None = fastest (no chunking), use 4-10 for very large datasets
    verbose=True
)
