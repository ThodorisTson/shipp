"""
Windpark Fryslån AEP Calculation - Advanced Models
===================================================

Uses state-of-the-art wake and turbulence models:
- Wake model: BastankhahGaussian (Gaussian wake deficit)
- Turbulence model: CrespoHernandez (wake-induced turbulence)
- Rotor averaging: GridRotorAvg(3×3 grid)

Improvements over IEA 740 baseline (NOJ):
- More realistic Gaussian wake shape vs linear expansion
- Wake-induced turbulence modeling for better recovery prediction
- Rotor disk averaging captures wind shear effects

Wind farm: 89 × 4.3 MW SWT-DD-130 turbines
Wind resource: 12-sector Weibull from ERA5 2024
Layout: Real coordinates from OpenStreetMap

Author: Thodoris  
Date: 2026-02-11
Comparison to: run_fryslan_iea_style.py (NOJ baseline)
"""
...
from py_wake.literature.gaussian_models import Bastankhah_PorteAgel_2014  # Changed from IEA
from py_wake.turbulence_models.crespo import CrespoHernandez    # Added as IEA had no turbulance
from py_wake.rotor_avg_models import EqGridRotorAvg  # Equal-spaced grid
...
print("=" * 70)
print("WINDPARK FRYSLÅN - AEP CALCULATION (ADVANCED MODELS)")
print("=" * 70)
print("\nModels:")
print("  Wake:       BastankhahGaussian")
print("  Turbulence: CrespoHernandez")
print("  Rotor Avg:  GridRotorAvg(3×3)")
print("=" * 70)
...
print(f"\n" + "=" * 70)
print("RUNNING PYWAKE BASTANKHAH-GAUSSIAN + CRESPO-HERNANDEZ MODEL")
print("=" * 70)

# Bastankhah-Porte-Agel 2014 model (literature version) with Crespo-Hernandez turbulence and rotor averaging
bastankhah = Bastankhah_PorteAgel_2014(
    site, 
    windTurbines,
    k=0.032,  # Wake expansion coefficient (default from Bastankhah & Porte-Agel 2014)
    turbulenceModel=CrespoHernandez(),      # Wake-induced turbulence!
    rotorAvgModel=EqGridRotorAvg(3)  # 3×3 equal-spaced grid)
)

print(f"\nBastankhah-Porte-Agel 2014 parameters:")
print(f"  Wake model: Bastankhah-Porte-Agel 2014 (literature-validated)")
print(f"  Wake expansion: k = 0.032 (offshore, from paper)")
print(f"  Turbulence model: CrespoHernandez (wake-induced TI)")
print(f"  Rotor averaging: EqGridRotorAvg(3×3) - 9 points")
print(f"\nModel improvements over NOJ:")
print(f"  ✓ Gaussian wake shape (more realistic)")
print(f"  ✓ Wake-induced turbulence (better recovery)")
print(f"  ✓ Rotor disk averaging (wind shear effects)")
...
if expected_cf_min <= cap_factor <= expected_cf_max:
    ...
    print(f"\nNote: This uses advanced models (Bastankhah + Crespo)")
    print(f"      Expected to be 3-7% higher than NOJ baseline (35.67%)")
else:
...
print(f"  Gaussian wake shape: More realistic than linear (NOJ)")
print(f"  Wake-induced TI: Better wake recovery prediction")
print(f"  Rotor averaging: Captures wind shear effects")
