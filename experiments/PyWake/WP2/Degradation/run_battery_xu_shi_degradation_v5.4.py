...
HORIZON
───────
discount_weights() sums years 1..N-1, matching the SHIPP kernel factor npf.npv(r, ones(n_year)) - 1, which drops the final year — a known off-by-one flagged to the 
supervisor. The convention lives in one place in wp2_econ.py and flips there if confirmed.
...
FILE_TAG = "v54"   # was "v53"

# ── v5.4 DoD sweep configuration ─────────────────────────────────────────────
SWEEP_KIND = "dod"          # "dod" (v5.4) | "ecap" (legacy v5.3 E sweep)

# (E, P) FIXED for the DoD study, as an OVERWRITE of the YAML read. Never edit the YAML per point. None on either falls back to the YAML value.
...
def _run_multiyear(
    ...
    for year in range(1, N_YEARS): # 19 revenue years (SHIPP convention)
        ...
        # All three come from the same dispatch at no extra cost. revenue_annual returns arbitrage + marginal; total is reconstructed as marginal + wind-only.
    ...
    #   Same as kernel's own NPV, but with proper per-year discounting. The gradient (rc_e_cap1) validates against this basis.
...
def _dod_points() -> List[Tuple[str, float, float]]:
    """(series_tag, center, width) for both series, de-duped on the shared 10-90
    anchor. Returns 8 points."""
    pts  = [("width", DOD_WIDTH_SERIES_CENTER, round(w, 6)) for w in DOD_WIDTH_SERIES]
    pts += [("center", round(c, 6), DOD_CENTER_SERIES_WIDTH) for c in DOD_CENTER_SERIES]
    seen, uniq = set(), []
    for tag, c, w in pts:
        key = (round(c, 6), round(w, 6))
        if key not in seen:
            seen.add(key)
            uniq.append((tag, c, w))
    return uniq
...
def _window_diagnostics(soc_year1, e_cap_nominal: float) -> Tuple[float, float, int, float]:
    """Year-1 observed cycle-depth diagnostics for one DoD window.

    Returns (mean_dod_wt, frac_weight_below_convex, n_cycles, max_dod_obs). frac_weight_below_convex is the load-bearing number: the share of cycle weight valued by
    an EXTRAPOLATED Phi (delta < 0.1437), where Xu is non-convex and the fit does not apply. Largest at narrow windows, which is exactly the width series.
    """
    ...
    wsum = float(cnts.sum())
    if wsum <= 0.0:
        return 0.0, 0.0, len(cyc), float(dods.max())
    mean_dod   = float(np.average(dods, weights=cnts))
    frac_below = float(cnts[dods < LMO_NONCONVEX_DELTA].sum() / wsum)
...
def _run_dod_sweep(
    ...
    Per window: derive (soc_min, soc_max), refit the convex Phi for that width, run the unchanged multi-year loop, and capture DoD -> revenue -> EoL plus the 
    fit/extrapolation diagnostics. dod (kernel floor) stays 1 - soc_min; soc_max1 stays soc_max (kernel lines 168/171).
    """
    n = len(price_eur)
    ...
    for idx, (tag, center, width) in enumerate(points):
        ...
        mean_dod_obs, frac_below, n_cyc, max_dod_obs = _window_diagnostics(
            my["annual_soc"][0], e_cap_fixed
        )
        fd_yr1     = my["annual_fd"][0][0]
        first_repl = my["replacement_years"][0] if my["replacement_years"] else None
        rev_fade   = (my["annual_revenue_bat_eur"][-1] /
                      my["annual_revenue_bat_eur"][0] - 1) * 100 \
                     if my["annual_revenue_bat_eur"][0] != 0 else 0.0

        results.append({
            "series": tag, "center": center, "width": width,
            ...
            "frac_below_convex": frac_below, "n_cycles_yr1": n_cyc,
            "elapsed_s": elapsed,
        })

        flag = "  <-- check: heavy sub-0.15 extrapolation" if frac_below > 0.25 else ""
        print(f"  NPV_bat={my['npv_bat_multiyear_EUR']*1e-6:>7.1f}M  "
              f"fd={fd_yr1:.5f}  repl={my['n_replacements']}  "
              f"SoH_end={my['final_soh_pct']:.1f}%  "
              f"mean_dod={mean_dod_obs:.3f}  frac<0.15={frac_below:.2f}{flag}  "
              f"t={elapsed:.1f}s")
...
def main() -> None:
    ...
    # ── Legacy E_cap sweep (v5.3 path) ──
    print(f"\n[4/4] Running E_cap sweep...")
    results = _run_ecap_sweep(
        power_wind_MW=power_wind_MW, price_eur=price_eur, p_max_MW=p_max_MW,
        p_cap_MW=p_cap_sweep, rte_ac=rte_ac,
        e_cost_EUR_per_MWh=e_cost_EUR_per_MWh, p_cost_EUR_per_MW=p_cost_EUR_per_MW,
        repl_e_EUR_per_MWh=repl_e_EUR_per_MWh, repl_p_EUR_per_MW=repl_p_EUR_per_MW,
        bat_params=bat_params, shi_fit=shi_fit, soc_min=soc_min, soc_max=soc_max,
    )
    _print_sweep_report(results)
    _save_sweep_csv(results, p_cap_sweep)
    _save_soh_trajectory_csv(results, p_cap_sweep)
