...
try:
    ...
    _C_NEUT = TUDELFT['navy']
    def _fig_size(w, aspect=0.7): return _figsize(w, aspect=aspect)
except ImportError:
    _C_PASS='#27ae60'; _C_FAIL='#e74c3c'; _C_NEUT='#2c7bb6'
    FS_BASE=10; FS_ANNOT=8
    def _fig_size(w, aspect=0.7): return (15*w, 15*w*aspect)
...
# ── 5. Run half-cycle attribution validation ───────────────────────────
    ...
    # ── Plot ──────────────────────────────────────────────────────────────
    fig, axes = plt.subplots(2, 3, figsize=_fig_size(1.0, aspect=0.62))

    def _text_panel(ax, lines):
        ax.axis("off")
        for j, (text, col, bold) in enumerate(lines):
            ax.text(0.5, 0.78 - j * 0.20, text, ha="center", va="center",
                    fontsize=FS_ANNOT if not bold else FS_BASE + 1, color=col,
    ...
    # P1 — Intra-half-cycle uniformity
    _text_panel(axes[0, 0], [
        (f"Tested: {r1['n_tested']} half-cycles", "black", False),
        (f"Max spread = {r1['max_spread']:.2e}", _C_NEUT, True),
        ("✓ Perfectly uniform" if r1["n_pass"] == r1["n_tested"] else "✗ FAIL",
         _C_PASS if r1["n_pass"] == r1["n_tested"] else _C_FAIL, True),
    ])
    ...
    if len(ratios2) > 0 and np.ptp(ratios2) < 1e-10:
        _text_panel(axes[0, 1], [
            (f"All {len(ratios2)} cycles", "black", False),
            (f"ratio = {ratios2.mean():.6f}", _C_NEUT, True),
            ("✓ Exact formula match" if p2_pass else "✗ FAIL",
             _C_PASS if p2_pass else _C_FAIL, True),
        ])
    else:
        axes[0, 1].hist(ratios2, bins=40, color=_C_NEUT, alpha=0.82,
                        edgecolor="white", lw=0.3)
        axes[0, 1].axvline(1.0, color=_C_FAIL, lw=1.8, ls="--")
        axes[0, 1].set_xlabel("Measured / Expected subgrad")
        axes[0, 1].set_ylabel("Count")

    # P3 — Zero outside ranges
    n_cov = r3["n_covered"]; n_unc = r3["n_uncovered"]
    bars_p3 = axes[0, 2].bar(
        ["Covered", "Uncovered"], [n_cov, n_unc],
        color=[_C_PASS if r3["pass"] else _C_FAIL, "lightgray"],
        alpha=0.85, width=0.45)
    for rect, val in zip(bars_p3, [n_cov, n_unc]):
        if val > 0:
            axes[0, 2].text(rect.get_x() + rect.get_width() / 2,
                            rect.get_height() + max(n_cov, n_unc) * 0.01,
                            str(val), ha="center", va="bottom",
                            fontsize=FS_ANNOT, fontweight="bold")
    axes[0, 2].set_ylabel("Timestep count")

    # P4 — Junction boundary convention
    df4 = r4["df"].dropna(subset=["ratio"])
    if len(df4) == 0:
        _text_panel(axes[1, 0], [("No junctions found", "gray", False)])
    elif np.ptp(df4["ratio"].values) < 1e-10:
        _text_panel(axes[1, 0], [
            (f"Junctions: {r4['n_tested']}", "black", False),
            (f"ratio = {df4['ratio'].mean():.6f}", _C_NEUT, True),
            ("✓ Correct convention" if r4["n_pass"] == r4["n_tested"] else "✗ FAIL",
             _C_PASS if r4["n_pass"] == r4["n_tested"] else _C_FAIL, True),
        ])
    else:
        axes[1, 0].hist(df4["ratio"].values, bins=30, color=_C_NEUT,
                        alpha=0.82, edgecolor="white", lw=0.3)
        axes[1, 0].axvline(1.0, color=_C_FAIL, lw=1.8, ls="--")
        axes[1, 0].set_xlabel("Junction ratio")
        axes[1, 0].set_ylabel("Count")

    # P5 — Global attribution sum conservation
    vals = [r5["measured_sum"], r5["expected_fp_sum"], r5["expected_all"]]
    xlabels = ["Measured\n(timestep)", "Expected\n(fingerprint)", "Expected\n(cycle)"]
    bars_p5 = axes[1, 1].bar(xlabels, vals,
                              color=[_C_NEUT, _C_PASS, _C_FAIL], alpha=0.85, width=0.45)
    for rect, val in zip(bars_p5, vals):
        axes[1, 1].text(rect.get_x() + rect.get_width() / 2,
                        rect.get_height() * 1.02, f"{val:.3e}",
                        ha="center", va="bottom", fontsize=FS_ANNOT, fontweight="bold")
    axes[1, 1].set_ylabel("Sum of contributions")
    ...
    for j, (label, passed) in enumerate(results):
        col = _C_PASS if passed else _C_FAIL
        axes[1, 2].text(0.08, 0.82 - j * 0.15,
                        f"{'✓' if passed else '✗'}  {label}",
                        transform=axes[1, 2].transAxes,
                        fontsize=FS_BASE, color=col,
