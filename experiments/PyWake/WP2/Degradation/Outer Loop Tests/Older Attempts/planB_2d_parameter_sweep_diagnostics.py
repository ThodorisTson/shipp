...
P_CAP_GRID = [225, 275]                      # 2 P; pairs with each E
...
def load_data(n_hours: int) -> Tuple[np.ndarray, np.ndarray, dict]:
    ...
    params = {
        ...
        "eta_in":  (rte_dc * pcu_eff**2) ** 0.5, # Symmetric split of the round-trip loss across charge and discharge.
        "eta_out": (rte_dc * pcu_eff**2) ** 0.5, # eta_in = eta_out = sqrt(round_trip), so eta_in*eta_out = round_trip is preserved (same economics) but charge is no longer modeled lossless.
...
def evaluate_single_ecap(
    ...
    # Source the no-degradation battery NPV directly from SHIPP rather than rebuilding it. a_npv is: battery-only, arbitrage revenue (dot(price,
    # storage_p)), capex netted internally, discounted on SHIPP's k=1..n_year-1 convention (19 revenue years for n_year=20). get_added_npv returns MUSD.
    a_npv_usd = os_res.get_added_npv(DISCOUNT_RATE, N_YEARS) * 1e6   # MUSD → USD

    # SHIPP's effective annuity factor: sum_{k=1}^{N_YEARS-1} (1+r)^-k.
    # Used ONLY to discount the degradation cost on the SAME horizon as a_npv, so the subtraction a_npv - deg_cost is internally consistent.
    factor = npf.npv(DISCOUNT_RATE, np.ones(N_YEARS)) - 1

    # Battery capex (USD) — reference/reporting only; a_npv already nets it, but the degradation-cost formula below needs e_cost * e_cap.
    capex_usd = p_cost * p_cap + e_cost * e_cap

    # ── Arbitrage revenue (matches SHIPP a_npv; reporting cross-check) ────
    revenue_arb_usd = 365.0 * 24.0 / n * factor * float(
        np.dot(price_eur * EUR_TO_USD, p_vec)) * DT

    # ── Marginal revenue (total plant − wind-only): captures curtailment ──
    # recovery on top of arbitrage. Same 19-yr factor as a_npv so the two NPVs differ ONLY by revenue definition, not horizon.
    price_used  = price_eur * EUR_TO_USD
    p_prod      = np.array(os_res.production_p[0].data, dtype=float)   # wind after curtailment
    wind_no_bat = np.minimum(power_wind, p_max)                       # wind-only baseline
    revenue_marg_usd = 365.0 * 24.0 / n * factor * float(
        np.dot(price_used, p_prod + p_vec) - np.dot(price_used, wind_no_bat)) * DT

    # Marginal NPV baseline = marginal revenue − capex (same capex a_npv nets).
    npv_marg_usd = revenue_marg_usd - capex_usd

    # Curtailment-recovery value = the gap between the two baselines.
    curtailment_value_usd = npv_marg_usd - a_npv_usd

    # keep `revenue_usd` name alive for any downstream code that reads it
    revenue_usd = revenue_arb_usd

# LOSS + SoC CONSERVATION CHECK — reads efficiencies from the solved Storage objects so it always matches what was actually built.
    for u in range(len(os_res.losses)):
        los_u  = np.asarray(os_res.losses[u])
        p_u    = np.asarray(os_res.storage_p[u].data)
        stor_u = os_res.storage_list[u]
        ein, eout_u, dt_u = stor_u.eff_in, stor_u.eff_out, os_res.storage_p[u].dt
        win, wout = p_u < 0, p_u >= 0
        err_in  = np.max(np.abs(los_u[win]  + (1 - ein) * p_u[win] * dt_u))        if win.any()  else 0.0
        err_out = np.max(np.abs(los_u[wout] - (1 - eout_u)/eout_u * p_u[wout] * dt_u)) if wout.any() else 0.0
        print(f"  [loss chk unit{u}] e_cap={stor_u.e_cap:.0f} "
              f"err_in={err_in:.3e} err_out={err_out:.3e} "
              f"eff_in={ein:.3f} eff_out={eout_u:.3f}")

    # Independent SoC conservation (does NOT use os_res.losses):
    e_arr = np.array(os_res.storage_e[0].data)
    p_arr = np.array(os_res.storage_p[0].data)
    ein0, eout0 = os_res.storage_list[0].eff_in, os_res.storage_list[0].eff_out
    dE_expected = np.where(p_arr <= 0, -DT * ein0 * p_arr, -DT * (1.0/eout0) * p_arr)
    soc_resid = np.max(np.abs(np.diff(e_arr) - dE_expected[:-1]))
    print(f"  [soc conservation] max_resid={soc_resid:.3e}  (want ~1e-6)")

    # locate the worst SoC-conservation violations
    dE_act  = np.diff(e_arr)
    err_vec = np.abs(dE_act - dE_expected[:-1])
    e_max   = os_res.storage_list[0].e_cap
    for i in np.argsort(err_vec)[-5:]:
        print(f"    t={i}: dE_act={dE_act[i]:+.2f} dE_exp={dE_expected[i]:+.2f} "
              f"p={p_arr[i]:+.2f} e[i]={e_arr[i]:.1f} e[i+1]={e_arr[i+1]:.1f} "
              f"e_cap={e_max:.0f} (e/e_cap={e_arr[i]/e_max:.3f})")

    # ── Option A: reconstruct true SoC from dispatch power, compare to kernel ──
    if PLOT_SOC_COMPARE and abs(e_cap - 1500) < 1 and abs(p_cap - 225) < 1:  # stress point (largest anomaly)
        soc_lo, soc_hi = e_cap * soc_min, e_cap * soc_max
        e_kernel = np.array(os_res.storage_e[0].data, dtype=float)
        nstep    = len(e_kernel)                        # = n (8760)
        e_recon  = np.empty(nstep)
        e_recon[0] = e_kernel[0]                         # same start as kernel
        for i in range(nstep - 1):                       # stop at nstep-2 → writes up to nstep-1
            step = (-DT * eta_in * p_vec[i]) if p_vec[i] <= 0 else (-DT / eta_out * p_vec[i])
            e_recon[i+1] = np.clip(e_recon[i] + step, soc_lo, soc_hi)

        # per-step CSV for eyeball inspection
        import csv as _csv
        from datetime import datetime as _dt
        _soc_ts = _dt.now().strftime("%Y%m%d_%H%M%S")
        csv_p = OUTPUT_DIR / f"soc_compare_E{e_cap:.0f}_P{p_cap:.0f}_{_soc_ts}.csv"
        with open(csv_p, "w", newline="", encoding="utf-8") as fh:
            wr = _csv.writer(fh)
            wr.writerow(["t", "p_dispatch_MW", "e_kernel_MWh", "e_recon_MWh",
                         "diff_MWh", "dE_kernel", "dE_recon", "dE_power_implied",
                         "at_bound"])
            for i in range(nstep - 1):
                step = (-DT * eta_in * p_vec[i]) if p_vec[i] <= 0 else (-DT / eta_out * p_vec[i])
                dE_k = e_kernel[i+1] - e_kernel[i]
                dE_r = e_recon[i+1]  - e_recon[i]
                at_b = (abs(e_kernel[i] - soc_hi) < 1e-6) or (abs(e_kernel[i] - soc_lo) < 1e-6)
                wr.writerow([i, round(p_vec[i], 3), round(e_kernel[i], 3),
                             round(e_recon[i], 3), round(e_kernel[i] - e_recon[i], 3),
                             round(dE_k, 3), round(dE_r, 3), round(step, 3), int(at_b)])
        print(f"  [soc compare] CSV: {csv_p.name}  max|diff|={np.max(np.abs(e_kernel-e_recon)):.1f} MWh")

        import matplotlib.pyplot as plt
        fig, ax = plt.subplots(2, 1, figsize=(14, 7), sharex=True)
        ax[0].plot(e_kernel, color="#b5351b", lw=0.8, label="Kernel SoC (relaxed LP)")
        ax[0].plot(e_recon,  color="#2166ac", lw=0.8, label="Reconstructed SoC (Option A)")
        ax[0].axhline(soc_hi, ls="--", c="gray", lw=0.6); ax[0].axhline(soc_lo, ls="--", c="gray", lw=0.6)
        ax[0].set_ylabel("SoC [MWh]"); ax[0].legend(loc="upper right", fontsize=8)
        ax[0].set_title(f"SoC comparison  E={e_cap:.0f} P={p_cap:.0f}  "
                        f"(max|diff|={np.max(np.abs(e_kernel-e_recon)):.1f} MWh)")
        ax[1].plot(e_kernel - e_recon, color="#444", lw=0.6)
        ax[1].set_ylabel("Kernel − Recon [MWh]"); ax[1].set_xlabel("Hour"); ax[1].axhline(0, c="gray", lw=0.5)
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / f"soc_compare_E{e_cap:.0f}_P{p_cap:.0f}_{_soc_ts}.png", dpi=130)
        plt.close(fig)

        # ── DECISIVE CHECK: does the SoC difference change fd? ──
        # Run the SAME degradation call on both traces; compare cycle-only fd (calendar term is SoC-independent so it can't differ).
        xu_kernel = analyze_degradation(
            storage_p=p_vec.tolist(), storage_e=e_kernel.tolist(),
            e_cap_nominal=e_cap, battery_params=params["bat_params"],
            dt_hours=DT, T_cell_C=params["T_cell_C"],
        )
        xu_recon = analyze_degradation(
            storage_p=p_vec.tolist(), storage_e=e_recon.tolist(),
            e_cap_nominal=e_cap, battery_params=params["bat_params"],
            dt_hours=DT, T_cell_C=params["T_cell_C"],
        )
        fdk, fdr = xu_kernel["fd_cycle"], xu_recon["fd_cycle"]
        rel = abs(fdk - fdr) / fdk * 100 if fdk else 0.0
        nck = len(rainflow_cycle_counting(e_kernel, e_cap))
        ncr = len(rainflow_cycle_counting(e_recon, e_cap))
        print(f"  [fd compare] fd_cycle kernel={fdk:.6f}  recon={fdr:.6f}  "
              f"rel_diff={rel:.3f}%   n_cycles kernel={nck} recon={ncr}")
        (OUTPUT_DIR / f"soc_compare_E{e_cap:.0f}_P{p_cap:.0f}_{_soc_ts}_fd.txt").write_text(
            f"E={e_cap:.0f} P={p_cap:.0f}\n"
            f"fd_cycle kernel={fdk:.6f}\nfd_cycle recon={fdr:.6f}\n"
            f"rel_diff={rel:.3f}%\nn_cycles kernel={nck} recon={ncr}\n"
            f"max|SoC diff|={np.max(np.abs(e_kernel-e_recon)):.1f} MWh\n",
            encoding="utf-8")
    ...
    npv_no_deg      = a_npv_usd                       # arbitrage baseline (= SHIPP a_npv)
    npv_with_xu     = a_npv_usd   - deg_cost_xu        # "physical accurate" (full Xu)
    npv_with_shi    = a_npv_usd   - deg_cost_shi       # "polynomial" (Shi-fit + Xu calendar)

    npv_marg_no_deg = npv_marg_usd                    # marginal baseline
    npv_marg_xu     = npv_marg_usd - deg_cost_xu
    npv_marg_shi    = npv_marg_usd - deg_cost_shi
    ...
    return {
        ...
        "revenue_marg_usd": revenue_marg_usd,
        "curtailment_value_usd": curtailment_value_usd,
        ...
        for p_cap in P_CAP_GRID:
            ...
            if is_pruned:
                ...
                r_pruned = {
                    ...
                    "pruned": True,
                    "revenue_usd": float("nan"), "revenue_marg_usd": float("nan"),
                    "capex_usd": (params["p_cost_usd_per_mw"] * p_cap_eff
                                  + params["e_cost_usd_per_mwh"] * e_cap),
                    "npv_no_deg": float("nan"), "npv_with_xu": float("nan"),
                    "npv_with_shi": float("nan"),
                    "npv_marg_no_deg": float("nan"), "npv_marg_xu": float("nan"),
                    "npv_marg_shi": float("nan"), "curtailment_value_usd": float("nan"),
