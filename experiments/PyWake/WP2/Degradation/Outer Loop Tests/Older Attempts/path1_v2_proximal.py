"""Path 1 v2 — Proximal bundle method with rainflow-based Shi degradation.

Purpose
-------
Solve the degradation-aware dispatch problem using proximal cutting
planes (proximal bundle method).  Adds a quadratic stabilization term
to prevent the LP from jumping between iterations, improving convergence
on the non-smooth rainflow degradation cost.

    1. Solve the QP dispatch (Gurobi)  →  SoC trajectory + dispatch
    2. Compute Shi degradation cost f_d via rainflow cycle counting
    3. Compute per-timestep subgradient (Shi Eq. 17–18)
    4. Add a cutting plane  θ ≥ f_d + g·(x − xᵏ)  to the QP
    5. Update proximal center and decay ρ
    6. Re-solve until convergence

The QP is solved with Gurobi throughout — the nonlinear degradation is
handled entirely through the external oracle + linear cuts + quadratic
proximal stabilization.

Improvement over Path 1 v1
    v1 used pure cutting planes which oscillated due to the non-smooth
    rainflow cost surface.  v2 adds proximal stabilization (ρ-weighted
    quadratic penalty) to damp oscillation and improve convergence.
...
Folder structure
    ...
    └── Outer Loop Tests/
        ├── path1_v2_proximal.py        ← this script
        ├── path1_cutting_plane.py      ← v1 (pure cutting planes)
        └── Results/

Usage
    python path1_v2_proximal.py                # 8760 h default
    python path1_v2_proximal.py --hours 168
    python path1_v2_proximal.py --full
    python path1_v2_proximal.py --c-deg 10
"""
...
from degradation_xu import rainflow_cycle_counting, phi_shi_with_stress
...
MAX_CUTS    = 40         # max cutting-plane iterations
CONV_TOL    = 0.005       # relative gap tolerance (0.001 = 0.1%, 0.005 = 0.5%)
RHO_INIT    = 50.0        # initial proximal weight (high = stay close)
RHO_DECAY   = 0.85        # multiply ρ by this each iteration
RHO_MIN     = 0.1         # stop decaying below this
...
def degradation_oracle(
    ...
    #   fd = Σ Φ(dod_i)·S_σ(σ_i)·S_T(T) × count_i  (consistent with subgradient)
    ...
    for c in cycles:
        ...
        sigma = c["soc_mean"]
        cnt = c["count"]
        fd += phi_shi_with_stress(dod, sigma, 25.0, k3, k4) * cnt
...
def solve_cutting_plane(
    ...
    # Proximal stabilization: mutable parameters for previous dispatch
    model.charge_prev    = pyo.Param(model.N, mutable=True, initialize=0, default=0)
    model.discharge_prev = pyo.Param(model.N, mutable=True, initialize=0, default=0)
    model.rho            = pyo.Param(mutable=True, initialize=rho_init)
    ...
    # Objective: max  Revenue − Capex − deg_scale × θ − ρ·‖x − x_prev‖²
    revenue_expr = rev_scale * sum(
        price_usd[i] * (model.discharge[i] - model.charge[i])
        for i in N
    )
    # Proximal term: -ρ · Σ [(charge - charge_prev)² + (discharge - discharge_prev)²]
    # This penalizes the LP for moving far from the previous iteration's dispatch.
    proximal_expr = model.rho * sum(
        (model.charge[i] - model.charge_prev[i])**2
      + (model.discharge[i] - model.discharge_prev[i])**2
        for i in N
    )

    model.obj = pyo.Objective(
        expr=revenue_expr - capex - deg_scale * model.theta - proximal_expr,
        sense=pyo.maximize,
    )
    ...
    for k in range(max_cuts + 1):
        ...
        # ── Update proximal center for next iteration ────────────────────
        for i in N:
            model.charge_prev[i]    = float(p_charge[i])
            model.discharge_prev[i] = float(p_discharge[i])

        # Decay ρ: loosen the rubber band as cuts accumulate
        current_rho = max(rho_init * (rho_decay ** (k + 1)), rho_min)
        model.rho.set_value(current_rho)
...
def compare_results(
    ...
    if cp["history"]:
        print(f"\n  {'Iter':>4s}  {'θ':>12s}  {'fd':>12s}  {'gap':>10s}  {'cycles':>6s}  {'ρ':>8s}")
        for h in cp["history"]:
            print(f"  {h['iter']:4d}  {h['theta']:12.8f}  {h['fd_actual']:12.8f}  "
                  f"{h['rel_gap']:+10.6f}  {h['n_cycles']:6d}  {h.get('rho', 0):8.2f}")
...
def save_csv(
    ...
    ts_csv = OUTPUT_DIR / f"path1_v2_timesteps_{n}h_{cdeg_tag}_{ts}.csv"
    ...
    sum_csv = OUTPUT_DIR / f"path1_v2_summary_{n}h_{cdeg_tag}_{ts}.csv"
    pd.DataFrame([summary]).to_csv(sum_csv, index=False)
    print(f"  Summary CSV:  {sum_csv.name}")

    # ── Text report ──────────────────────────────────────────────────────
    report_path = OUTPUT_DIR / f"path1_v2_report_{n}h_{cdeg_tag}_{ts}.txt"
    _write_report(report_path, lp, cp, params, n, c_deg_scale, max_cuts, conv_tol, rho_init, rho_decay, rho_min)
...
def _write_report(
    ...
    w(f"  rho_init:       {rho_init}")
    w(f"  rho_decay:      {rho_decay}")
    w(f"  rho_min:        {rho_min}")
    ...
    w(f"  {'Iter':>4s}  {'theta':>14s}  {'fd_actual':>14s}  {'gap':>12s}  "
      f"{'cycles':>6s}  {'rho':>8s}  {'time_s':>7s}")
    w(f"  {'─'*4}  {'─'*14}  {'─'*14}  {'─'*12}  {'─'*6}  {'─'*8}  {'─'*7}")
    for h in cp["history"]:
        w(f"  {h['iter']:4d}  {h['theta']:14.10f}  {h['fd_actual']:14.10f}  "
          f"{h['rel_gap']:+12.8f}  {h['n_cycles']:6d}  {h.get('rho', 0):8.2f}  {h['time_s']:7.2f}")
...
def plot_comparison(
    ...
    out = OUTPUT_DIR / f"path1_v2_comparison_{n}h_{cdeg_tag}_{ts_str}.png"
...
def main():
    ...
    print(f"    ρ_init:        {rho_init}")
    print(f"    ρ_decay:       {rho_decay}")
    print(f"    ρ_min:         {rho_min}")
    ...
    save_csv(lp, cp, params, power_wind, price_eur, n, c_deg, max_cuts, conv_tol, rho_init, rho_decay, rho_min)
    if not skip_plot:
