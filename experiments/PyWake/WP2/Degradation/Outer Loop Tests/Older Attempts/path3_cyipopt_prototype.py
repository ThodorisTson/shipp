...
_GRANDPARENT_DIR = _PARENT_DIR.parent
for p in [str(_PARENT_DIR), str(_GRANDPARENT_DIR)]:
...
# ═══════════════════════════════════════════════════════════════════════════
# Scipy trust-constr solver  (sparse Jacobian — scales to 8760)
# ═══════════════════════════════════════════════════════════════════════════

def solve_nlp_trustconstr(
    nlp: BatteryDispatchNLP,
    x0: Optional[np.ndarray] = None,
    max_iter: int = 500,
    tol: float = 1e-6,
    ...
    """Solve the NLP using scipy trust-constr with SPARSE constraint Jacobian.

    Unlike SLSQP (which allocates a dense n×n working matrix), trust-constr
    uses sparse linear algebra and L-BFGS Hessian approximation internally.
    Memory usage is O(n) not O(n²), so it scales to 26,000+ variables.
    """
    ...
    # Variable bounds as Bounds object
    bounds = Bounds(lb, ub)

    # Build SPARSE constraint Jacobian (constant — all constraints linear)
    n_cons = nlp.n_cons
    n_vars = nlp.n_vars
    A_sparse = sparse.csr_matrix(
        (nlp._jac_vals, (nlp._jac_rows, nlp._jac_cols)),
        shape=(n_cons, n_vars),
    )

    # All constraints are equalities: A @ x = 0
    linear_constraint = LinearConstraint(A_sparse, lb=0.0, ub=0.0)
    ...
    def _callback(xk, state):
        _state["iter"] += 1
        if verbose and _state["iter"] % 10 == 0:
            obj = state.fun if hasattr(state, 'fun') else nlp.objective(xk)
            constr_viol = state.constr_violation if hasattr(state, 'constr_violation') else 0
            print(f"    iter {_state['iter']:4d}  obj = {obj:+.6e}  "
                  f"constr_viol = {constr_viol:.2e}")
        return False  # don't stop

    if verbose:
        print(f"  Solving with scipy trust-constr  (n={n_vars}, m={n_cons})")
        nnz = len(nlp._jac_vals)
        density = nnz / (n_cons * n_vars) * 100
        print(f"  Jacobian: {nnz} nonzeros ({density:.3f}% dense)")
...
# ═══════════════════════════════════════════════════════════════════════════
# Unified solver: IPOPT → trust-constr (large) → SLSQP (small)
# ═══════════════════════════════════════════════════════════════════════════
...
def solve_nlp(
    ...
    """Solve the NLP — tries CyIpopt, then trust-constr or SLSQP by size."""
    # Try CyIpopt first (best solver if available)
    try:
    ...
    # Fallback: choose by problem size
    if nlp.n_vars > 2000:
        # Large-scale → trust-constr with sparse Jacobian
        if verbose:
            print(f"  CyIpopt not available — using scipy trust-constr "
                  f"(sparse, n={nlp.n_vars})")
        return solve_nlp_trustconstr(
            nlp, x0=x0, max_iter=max_iter, verbose=verbose,
        )
    else:
        # Small-scale → SLSQP (faster for small problems)
        if verbose:
            print(f"  CyIpopt not available — using scipy SLSQP "
                  f"(n={nlp.n_vars})")
        return solve_nlp_scipy(
            nlp, x0=x0, max_iter=max_iter, verbose=verbose,
        )
