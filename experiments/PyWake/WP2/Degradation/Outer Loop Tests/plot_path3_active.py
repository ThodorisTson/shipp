...
Usage (from the Outer Loop Tests folder):
  python plot_path3.py --scatter --annotate-scatter   # 356 active days, clip 150
  python plot_path3.py --scatter --all-days           # include the nine idle days
...
def plot_scatter(csv: Path, out: Path, annotate_days=None, net_clip: float = 150.0, active_only: bool = True):
    ...
    if active_only:
        rows = [r for r in rows if r["case"] not in IDLE_DAYS]
