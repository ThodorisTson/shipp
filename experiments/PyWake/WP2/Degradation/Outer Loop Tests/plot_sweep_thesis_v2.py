...
# SLIDE MODE
# ═══════════════════════════════════════════════════════════════════════════
# Thesis figures are read at arm's length on paper; slide figures are read
# from the back of a room. Same data, different sizing. Mirrors the SLIDE
# toggle already in plot_rainflow_explainer_v4.py.
#
# The important rule: draw the figure at the width it will be PLACED on the
# slide. Drawing larger and letting PowerPoint shrink it undoes the font bump
# exactly, which is how the current figures ended up looking small.
#
# SLIDE_SCALE multiplies the thesis_style sizes. 1.0 reproduces the thesis
# figure. Around 1.6 matches the sizing used before thesis_style existed.
SLIDE       = False     # True -> presentation sizing, writes *_slide.pdf/.png
SLIDE_SCALE = 1.6       # font multiplier on FS_BASE / FS_LABEL / FS_ANNOT
SLIDE_WIDTH_IN = 4.60   # width the heatmap occupies on the slide
SLIDE_ASPECT   = 0.74   # h/w of the CANVAS. The colourbar takes width
                        # from the axes, so a canvas near 0.74 gives a
                        # roughly square plotting box.

if SLIDE:
    FS_TICK  = FS_BASE  * SLIDE_SCALE
    FS_AXLAB = FS_LABEL * SLIDE_SCALE
    FS_NOTE  = FS_ANNOT * SLIDE_SCALE
    LW_AXES, LW_CONTOUR, LW_BOX = 0.9, 0.35, 1.8
    MS_STAR = int(340 * SLIDE_SCALE * 0.80)
    MS_DIAM = int( 90 * SLIDE_SCALE)
    MS_CIRC = int(110 * SLIDE_SCALE * 0.80)
    MS_DOT  = int(  7 * SLIDE_SCALE * 0.90)
    # Explicit tick steps, so labels land on sweep grid values.
    TICK_P_FULL, TICK_E_FULL = 50, 200     # coarse-domain heatmap
    TICK_P_ZOOM, TICK_E_ZOOM = 50, 50      # refined-domain heatmap
    SUFFIX  = "_slide"
    plt.rcParams.update({
        "font.size":       FS_TICK,
        "axes.labelsize":  FS_AXLAB,
    ...
    FS_TICK, FS_AXLAB, FS_NOTE = FS_BASE, FS_LABEL, FS_ANNOT
    LW_AXES, LW_CONTOUR, LW_BOX = 0.6, 0.20, 1.4
    MS_STAR, MS_DIAM, MS_CIRC, MS_DOT = 340, 90, 110, 5
    TICK_P_FULL = TICK_E_FULL = TICK_P_ZOOM = TICK_E_ZOOM = None
    SUFFIX  = ""


def _fig_size(frac_thesis: float, aspect: float):
    """Thesis: a fraction of the text width. Slide: the placed width."""
    if SLIDE:
        return (SLIDE_WIDTH_IN, SLIDE_WIDTH_IN * SLIDE_ASPECT)
    return figsize(frac_thesis, aspect=aspect)


def _slide_axes(ax, cb=None, xstep=None, ystep=None):
    """Tick rules that only apply in slide mode.

    Axis ticks are set by an explicit step, not by MaxNLocator. A locator
    picks round-looking values such as 160 or 240 that are not sweep grid
    points; a fixed step of 50 MWh reproduces the sampling.

    The colourbar locator is deliberately NOT touched. For a contourf plot
    matplotlib ticks the contour levels themselves and subsamples them, so a
    17-level fill gives 9 labelled values. Overriding it with MaxNLocator
    replaces the level boundaries with arbitrary round numbers and breaks the
    correspondence between a label and a band of colour.
    """
    if not SLIDE:
        return
    ...
    if cb is not None:
        ...
        cb.set_label("Lifetime NPV  (MEUR)", fontsize=FS_AXLAB)
...
def plot_full_heatmap(df: pd.DataFrame, scenario: str,
    ...
    if scenario != "no_deg":
        ...
        ax.scatter([Po], [Eo], marker="*", s=MS_STAR, c=C_SCENARIO[scenario],
    ...
    ax.scatter([Pn], [En], marker="D", s=MS_DIAM, c=C_NODEG,
    ...
    rect = Rectangle((ZOOM_P_MIN, ZOOM_E_MIN),
                      ...
                      linewidth=LW_BOX, edgecolor=TUDELFT["navy"],
    ...
    _slide_axes(ax, cb, xstep=TICK_P_FULL, ystep=TICK_E_FULL)
    _save(fig, f"fig_sweep_full_{scenario}{SUFFIX}", out_dir)
...
def plot_zoom_heatmap(df: pd.DataFrame, scenario: str,
    ...
    fig, ax = plt.subplots(figsize=_fig_size(0.68, 0.88))
    ...
    # Grid optimum. Omitted for no_deg: it coincides with the diamond below.
    if scenario != "no_deg":
        Eo, Po, No = grid_opt_all[scenario]
        ax.scatter([Po], [Eo], marker="*", s=MS_STAR, c=C_SCENARIO[scenario],
                   edgecolors="k", linewidths=1.0, zorder=6,
    ...
    # Quadratic optimum
    Es, Ps, Ns, is_max, r2 = quad if quad is not None else (0, 0, 0, False, 0)
    if is_max:
        ax.scatter([Ps], [Es], marker="o", s=MS_CIRC, facecolors="none",
    ...
    _slide_axes(ax, cb, xstep=TICK_P_ZOOM, ystep=TICK_E_ZOOM)
    _save(fig, f"fig_sweep_zoom_{scenario}{SUFFIX}", out_dir)
...
def main() -> None:
    RESULTS_DIR = Path(__file__).parent / "Plan B Results"
    ...
    # No-degradation over the SAME refined region, for a like-for-like
    # comparison against the Xu panel. Its own colour levels: on this grid the
    # no-deg surface spans roughly 106-133 MEUR against 39-57 for the degraded
    # models, so the shared scale would render it flat.
    nd = df_zoom.dropna(subset=[NPV_COL["no_deg"]])[NPV_COL["no_deg"]].values
    lo_n, hi_n = float(nd.min()), float(nd.max())
    pad_n = 0.02 * (hi_n - lo_n)
    nodeg_levels = np.linspace(lo_n - pad_n, hi_n + pad_n, 17)
    plot_zoom_heatmap(df_zoom, "no_deg", grid_zoom, None, OUT_DIR,
                      levels=nodeg_levels)
