"""WP2 battery optimization + Shi degradation,  v5.1 (UNIFIED, horizon-agnostic).

v5.1 builds on the validated v5 Castillo gradient and removes the 8760-hour restriction and the MULTI_YEAR flag.  The same logic now runs for ANY representative window
(30 days to a full year) over ANY project lifetime (1 to 20+ years).  v5's gradient validation (rc match, FD match) holds unchanged at n=8760; the sub-year path is 
validated separately via the [1.3] and [1.5] diagnostics at a shorter window.
...
2. Unified lifetime loop (no MULTI_YEAR flag)
   The degradation loop ALWAYS runs. Two orthogonal time concepts:
     - representative window  n  : actual price/wind data, 30 d to 1 yr. Annualised inside the loop by scale = 8760/n (continuous, correct for all n; =1 at n=8760).
     - project lifetime  N_YEARS : how many years to accumulate degradation, trigger replacements, and discount NPV.  N_YEARS=1 is the single-year case as a natural 
       special case — no separate code path.
   Annualisation acts on the LINEAR fd accumulator (Layer 1) only. The steep-first-year SEI nonlinearity lives in the cumulative-fd→SoH map (Layer 2) and is untouched, 
   so it is preserved at every window length.
...
--------------
  ...
  _run_multiyear()                — unified lifetime loop: per-year LP re-solve + degradation (annualised ×8760/n) + Castillo gradient (dRev/dDeg/lambda_E/dNPV/rc)
...
# ── v2 additions ──────────────────────────────────────────────────────────────
# MULTI_YEAR       = True     # MULTI_YEAR flag removed in v5.1: the lifetime loop always runs. Single-year behaviour is simply N_YEARS = 1.
N_YEARS          = 2       # project lifetime for multi-year loop (1 = single-year run)
...
def _fd_validate_degradation_term(
    ...
    Validates ONLY the degradation channel — the one term rc cannot check, since the LP is blind to degradation.

    Frozen dispatch: cycle ENERGY throughput is held fixed, so the fractional cycle depth scales as delta(E') = delta * E / E' = delta / (1 ± eps). We rebuild the 
    TOTAL degradation cost at E, E(1+eps), E(1-eps) using the SAME stress-weighted Shi cost as the analytic term, then central-difference.
    ...
    A PASS is FD ≈ analytic to a few % AND stable across the two eps values (Richardson sanity: shrinking eps should not change the estimate).
    """
    if len(dods) == 0:
    ...
    B_fac  = e_cost_USD_per_MWh * factor * scale # common prefactor: ×factor→lifetime-NPV, ×scale(=8760/n)→annualised. 
    # Identical to the gradient term's prefactor so analytic & FD match the ACTUAL dDegCost_dEcap at any window n (not just n=8760 where scale=1).
...
def _run_multiyear(
    ...
    for year in range(1, N_YEARS + 1):
        ...
        if os_yr.dual_prices is not None:                       # duals exist only if LP solved with return_duals=True
            ...
            # ═══════════════════════════════════════════════════════════════
            # OUTER-LOOP SIZING GRADIENT  dNPV/dE_cap   (Castillo et al. 2008)
            # ───────────────────────────────────────────────────────────────
            # Goal: how does lifetime NPV change if we add one MWh of capacity?
            #   dNPV/dE_cap = dRev/dE_cap  -  dDegCost/dE_cap  -  lambda_E
            # All three terms are assembled on ONE basis: lifetime-NPV, per MWh.
            # Validated: dRev & lambda_E vs Gurobi reduced cost (rc) to 6 s.f.; # dDegCost vs frozen-dispatch finite difference to 0.003%.
            # ═══════════════════════════════════════════════════════════════
            factor_yr        = npf.npv(discount_rate, np.ones(N_YEARS)) - 1   # NPV annuity: turns 1 yr of cash flow into lifetime PV
            dual_per_year_yr = dual_yr / factor_yr                            # de-annualised lower-bound dual (used only by the OLD scalar below)

            # ── [0] OLD fused scalar — DEPRECATED, kept only for continuity ──
            # Conceptually wrong: it dots the degradation subgradient against the revenue dual, mixing two unrelated channels. 
            # Retained at index [0] so legacy CSV columns/plots still populate. NOT used for decisions.
            dDeg_dDoD_yr     = -e_cap_yr * float(
                np.dot(sg_yr["subgrad_combined"], dual_per_year_yr)
            )

            # ── Cycle arrays for this year's frozen SoC trace ──
            dods_yr     = np.array([c["dod"]      for c in cycles_yr])   # cycle depths δ_i, FRACTION of capacity (0–1)
            cnts_yr     = np.array([c["count"]    for c in cycles_yr])   # rainflow weights: 1.0 full cycle, 0.5 half
            socm_yr     = np.array([c["soc_mean"] for c in cycles_yr])   # mean SoC per cycle (drives the S_σ stress term)
            mean_dod_yr = float(np.average(dods_yr, weights=cnts_yr)) if len(dods_yr) > 0 else 0.0

            # ── TERM 1: dRev/dE_cap  (EXACT — Castillo Theorem 1 / Example 5) ──
            # E_cap enters the LP ONLY through the SoC-bound constraint RHS:
            #     upper:  e_t ≤ E_cap·soc_max   →  chain coefficient +soc_max
            #     lower:  e_t ≥ E_cap·soc_min   →  chain coefficient -soc_min
            # The sensitivity of optimal revenue to a constraint RHS IS that constraint's dual (Castillo). 
            # Raising E_cap RAISES the ceiling (relaxes ≤ → +revenue) but also RAISES the floor (tightens ≥ → -revenue), 
            # hence opposite signs. The duals already carry the full 365·24/n·factor objective scaling, so this term is lifetime-NPV.
            dual_max_yr   = os_yr.dual_prices["dual_e_max1"]
            dRev_dEcap_yr = (
                soc_max * float(np.sum(dual_max_yr))     # upper bound: +soc_max
                - soc_min * float(np.sum(dual_yr))       # lower bound: -soc_min  (sign validated vs rc)
            )

            # ── TERM 2: dDegCost/dE_cap  (FROZEN-DISPATCH approximation) ──
            # "Frozen dispatch" = hold the optimal schedule (c_t,d_t) fixed and ask only how degradation COST moves with E_cap. 
            # We do NOT re-solve the LP. The Shi cycle cost is B·Φ(δ), with E entering ONLY inside δ because δ_i = (energy throughput
            # of cycle i)/E. Throughput is frozen, so:
            # dδ_i/dE = d(throughput/E)/dE = -throughput/E² = -δ_i/E → single product-rule term  dCost_i/dE = B·Φ'(δ_i)·(-δ_i/E).
            # Φ' uses phi_shi_prime_with_stress — the SAME kernel as the validated subgradient (incl. S_σ, S_T) — so the two can 
            # never silently diverge.
            #
            # SCALING (this is the unification fix): the reporting fd at line ~646 is annualised by  scale = 8760/n  (a 30-day window
            # ×12.17 → annual). The gradient differentiates that SAME fd, so it carries the SAME ×scale. Then ×factor_yr lifts 
            # the annual figure to lifetime-NPV, matching the revenue dual's basis. Both scale and factor act on the LINEAR fd 
            # accumulator (Layer 1) only — the steep-first-year SEI nonlinearity lives in the cumulative-fd→SoH map (Layer 2) 
            # downstream and is NOT touched here. At n=8760, scale=1 (no-op, preserves the validated full-year result exactly).
            if len(dods_yr) > 0:
                phi_prime_cyc = phi_shi_prime_with_stress(
                    dods_yr, socm_yr, T_cell_C, shi_fit.k3, shi_fit.k4
                )
                ddod_dEcap_yr     = -dods_yr / e_cap_yr      # dδ_i/dE = -δ_i/E  (frozen-dispatch chain factor)
                dDegCost_dEcap_yr = e_cost_USD_per_MWh * factor_yr * scale * float(
                    np.sum(cnts_yr * phi_prime_cyc * ddod_dEcap_yr)   # Σ over cycles, rainflow-weighted
                )
            else:
                dDegCost_dEcap_yr = 0.0 # no cycles → capacity doesn't change degradation

            # ── TERM 3: lambda_E  (capex of one extra MWh) ──
            # The LP objective's capex term is e_cost·e_cap1 (one-time, UNSCALED — no factor), so dObjective/dE_cap from 
            # capex = -e_cost. Hence lambda_E is just e_cost, NOT e_cost·factor. (This matches the kernel objective.)
            lambda_E_yr   = e_cost_USD_per_MWh

            # ── Assembled outer gradient ──
            dNPV_dEcap_yr = dRev_dEcap_yr - dDegCost_dEcap_yr - lambda_E_yr
            rc_e_cap_yr   = float(os_yr.dual_prices.get("rc_e_cap1", 0.0))  # solver's own dObj/dE_cap — independent oracle

            # ── STEP 1.5: validate dDegCost term once via frozen-dispatch FD (year 1) ──
            if year == 1:
            ...
            # ── STEP 1.3 BASIS DIAGNOSTIC (active for sub-year validation; comment out once trusted) ──
...
def main() -> None:
    ...
    if n != 8760:
        print(f"  ℹ  Window is {n} h ({n/24:.0f} days), not a full year. "
              f"Degradation & revenue will be annualised ×{8760/n:.3f} per project year.")
    ...
    # ── 6. Multi-year loop (unified: any window n, any lifetime N_YEARS) ────────────
    multiyear = None # No MULTI_YEAR flag: the loop always runs.  N_YEARS=1 is the single-year case;
    if True:  #N_YEARS>1 accumulates degradation/replacement over the project life.The representative window (n timesteps) is
        print(f"\n[6/6] Lifetime degradation loop " # annualised inside the loop by ×8760/n, so 30-day, 90-day, pr full-year 
              f"({N_YEARS} yr × {n/24:.0f}-day window, annualised ×{8760/n:.3f})...") # windows all follow identical logic.
        # Pass the representative window as-is; loop derives its length internally. 
