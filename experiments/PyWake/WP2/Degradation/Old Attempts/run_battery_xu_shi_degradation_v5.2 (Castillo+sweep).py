"""WP2 battery optimization + Shi degradation,  v5.2 (Castillo + Sweep).

v5.2 builds on the validated v5.1 Castillo gradient and adds two features:

  Phase 3-lite — Proper discounted lifetime NPV
  ───────────────────────────────────────────────
  v5.1 reported NPV = os_fixed.npv − PV(replacements), where os_fixed.npv uses year-1 revenue × factor as the lifetime revenue. This ignores the fact that later years earn LESS because capacity has faded.

  v5.2 extracts the per-year UNDISCOUNTED revenue from each LP solve (the dispatch is already re-solved with faded capacity), discounts each year individually, and assembles:

      NPV = −capex_initial + Σ_{t=1}^{N} rev_t / (1+r)^t − PV(replacements)

  This is correct and captures the revenue loss from capacity fade.

  Phase 2 — E_cap parameter sweep
  ────────────────────────────────
  Wraps the corrected multi-year loop in a sweep over E_cap values.
  At each point, records:
    - NPV_no_deg     :  no degradation (year-1 revenue × factor)
    - NPV_multiyear  :  proper discounted NPV with fade + replacements
    - NPV_planB      :  Plan B single-year extrapolation (for comparison)
    - annual fd, EFC, SoH trajectory, gradient

  The key thesis figure is NPV(E_cap) with three curves overlaid.

Inherited from v5.1
────────────────────
  All degradation machinery, dual-Φ framework, Castillo gradient, calendar correction, battery replacement logic — unchanged and validated.

No changes to:
  degradation_xu.py, degradation_shi.py, degradation_subgradient.py, kernel_pyomo.py, degradation_plots.py, degradation_plots_multiyear.py
"""
...
def _build_storage_year(
    ...
    return Storage(
        e_cap=e_cap_eff, p_cap=p_cap_MW, eff_in=1.0, eff_out=rte_ac,
...
# =============================================================================
# Multi-year loop  (v5.2: MODIFIED for proper discounted NPV)
# =============================================================================
...
def _run_multiyear(
    ...
    v5.2 additions vs v5.1:
      - Extracts per-year BATTERY-ONLY revenue from each LP solve. Battery revenue = total plant revenue − wind-only baseline.
        This captures the full marginal value of the battery including curtailment reduction, but excludes the wind farm's own revenue.
      - Assembles proper discounted lifetime NPV:
            NPV_battery = -capex + Σ rev_bat_t/(1+r)^t - PV(replacements)
      - Also computes Plan B (single-year extrapolation) NPV for comparison
      - Also computes no-degradation NPV from year-1 battery revenue
    """
    n           = len(wind_8760)
    ...
    for year in range(1, N_YEARS + 1):
        ...
        # ── v5.2: Extract per-year revenues ─────────────────────────────
        # Total plant revenue = price · (wind_after_curtailment + battery)
        # Battery marginal revenue = total − wind_only_baseline
        # This captures: arbitrage profit + curtailment reduction benefit
        p_prod_yr = np.array(os_yr.production_p[0].data, dtype=float)
        p_bat_yr  = np.array(os_yr.storage_p[0].data,    dtype=float)
        total_export_yr = p_prod_yr + p_bat_yr
        rev_total_yr = (365.0 * 24.0 / n) * float(np.dot(price_eur, total_export_yr)) * dt
        rev_bat_yr   = rev_total_yr - rev_wind_only   # battery's marginal contribution
        ...
        if os_yr.dual_prices is not None:
            ...
            sg_yr     = compute_subgradient(
                ...
                eff_in=1.0, eff_out=rte_ac, shi_fit=shi_fit,
            )
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # v5.2: PROPER DISCOUNTED LIFETIME NPV — DUAL BASIS
    # ═════════════════════════════════════════════════════════════════════════
    capex_initial     = e_cap_nominal * e_cost_EUR_per_MWh + p_cap_MW * p_cost_EUR_per_MW
    capex_replacement = repl_e_EUR_per_MWh * e_cap_nominal + repl_p_EUR_per_MW * p_cap_MW  # energy + power expansion (reuses BOP; PCS bundled w/ battery replacement)
    factor            = npf.npv(discount_rate, np.ones(N_YEARS + 1)) - 1

    # PV of replacement costs (shared by both bases)
    pv_replacements = sum(
        capex_replacement / (1.0 + discount_rate) ** yr
    ...
    pv_rev_bat = sum(
        rev / (1.0 + discount_rate) ** yr
        for yr, rev in enumerate(annual_revenue, 1)
    )
    ...
    deg_cost_planB = fd_yr1_total * repl_e_EUR_per_MWh * e_cap_nominal * factor  # deg valued at energy-expansion cost
    ...
    pv_rev_total = sum(
        rev / (1.0 + discount_rate) ** yr
        for yr, rev in enumerate(annual_revenue_total, 1)
    )
...
def _save_sweep_csv(results: List[Dict], p_cap_MW: float) -> Path:
    ...
    csv_path = RESULTS_DIR / f"v52_sweep_{run_ts}_P{int(p_cap_MW)}MW.csv"
...
def _save_soh_trajectory_csv(results: List[Dict], p_cap_MW: float) -> Path:
    ...
    path = RESULTS_DIR / f"v52_soh_trajectory_{run_ts}_P{int(p_cap_MW)}MW.csv"
...
def _find_latest_csv() -> Optional[Path]:
    """Find the most recent v52_sweep CSV in RESULTS_DIR."""
    csvs = sorted(RESULTS_DIR.glob("v52_sweep_*.csv"), key=lambda p: p.stat().st_mtime)
...
def _plot_sweep(results: List[Dict]) -> None:
    ...
    fig.suptitle(
        f"v5.2 Sweep: Battery NPV vs Capacity  "
    ...
    out = PLOTS_DIR / f"v52_sweep_{run_ts}.png"
...
def _plot_planB_comparison(results: List[Dict]) -> None:
    ...
    plt.tight_layout()
    out = PLOTS_DIR / f"v52_planB_comparison_{run_ts}.png"
    fig.savefig(out, dpi=200, bbox_inches="tight", facecolor=BG)
    print(f"  ✓ Plot: {out.name}")
    if show_plots:
        plt.show()
...
def main() -> None:
    ...
    print("=" * 80)
    print("WP2 BATTERY OPTIMIZATION + DEGRADATION  v5.2 (Castillo + Sweep)")
    ...
    # ── Save config sidecar ──
    config_path = RESULTS_DIR / f"v52_sweep_{run_ts}_P{int(p_cap_sweep)}MW.json"
    with open(config_path, "w", encoding="utf-8") as f:
        json.dump({
            "version":       "v5.2",
            "revenue_basis": "battery-only (total plant minus wind-only baseline)",
    ...
    print(f"\n{'═'*80}")
    print("✓ v5.2 COMPLETE (Castillo + Sweep)")
    print(f"{'═'*80}")


if __name__ == "__main__":
    log_path = RESULTS_DIR / f"v52_log_{run_ts}.txt"
