...
def _dod_points() -> List[Tuple[str, float, float]]:
    """(series_tag, center, width) for both series, de-duped on the shared 10-90 anchor.
    The shared point is tagged "both"; the CSV writer emits it into each series so the
    center curve keeps its middle point and the width curve keeps its top point, with no
    extra LP solve. Returns 8 unique points."""
    cw = round(DOD_WIDTH_SERIES_CENTER, 6)
    ww = round(DOD_CENTER_SERIES_WIDTH, 6)
    width_keys  = {(cw, round(w, 6)) for w in DOD_WIDTH_SERIES}
    center_keys = {(round(c, 6), ww) for c in DOD_CENTER_SERIES}
    ordered = ([(cw, round(w, 6)) for w in DOD_WIDTH_SERIES]
               + [(round(c, 6), ww) for c in DOD_CENTER_SERIES])
    seen, uniq = set(), []
    for c, w in ordered:
        if (c, w) in seen:
            continue
        seen.add((c, w))
        in_w, in_c = (c, w) in width_keys, (c, w) in center_keys
        tag = "both" if (in_w and in_c) else ("width" if in_w else "center")
        uniq.append((tag, c, w))
    return uniq


def _window_diagnostics(soc_year1, e_cap_nominal: float, k4: float
                        ) -> Tuple[float, float, int, float, float]:
    """Year-1 observed cycle-depth diagnostics for one DoD window.

    Returns (mean_dod_wt, frac_weight_below_convex, n_cycles, max_dod_obs,
    frac_fd_below_convex).
    frac_weight_below_convex: share of cycle COUNT below delta < 0.1437, where Xu is
    non-convex and the fitted Phi is extrapolated.
    frac_fd_below_convex: share of cycling FD CONTRIBUTION from those same cycles, using
    the fitted exponent (contribution per cycle ~ count * delta**k4; k3 cancels in the
    ratio). Because k4 > 1 down-weights shallow cycles, this is much smaller than the
    count fraction and is the defensible measure of how much reported cycling fd rests on
    extrapolation.
    """
    ...
    wsum = float(cnts.sum())
    if wsum <= 0.0:
        return 0.0, 0.0, len(cyc), float(dods.max()), 0.0
    below      = dods < LMO_NONCONVEX_DELTA
    mean_dod   = float(np.average(dods, weights=cnts))
    frac_below = float(cnts[below].sum() / wsum)
    fd_contrib = cnts * np.power(dods, k4)          # proportional to cycling fd (k3 cancels)
    fd_total   = float(fd_contrib.sum())
    frac_fd_below = float(fd_contrib[below].sum() / fd_total) if fd_total > 0.0 else 0.0
    return mean_dod, frac_below, len(cyc), float(dods.max()), frac_fd_below
...
def _run_dod_sweep(
    ...
    for idx, (tag, center, width) in enumerate(points):
        ...
        mean_dod_obs, frac_below, n_cyc, max_dod_obs, frac_fd_below = _window_diagnostics(
            my["annual_soc"][0], e_cap_fixed, shi_fit_w.k4
        )
        ...
        print(f"  NPV_bat={my['npv_bat_multiyear_EUR']*1e-6:>7.1f}M  "
              ...
              f"mean_dod={mean_dod_obs:.3f}  "
              f"cyc<0.15={frac_below:.2f}  fd<0.15={frac_fd_below:.2f}{flag}  "
              f"t={elapsed:.1f}s")
...
def _save_dod_sweep_csv(results: List[Dict], e_cap: float, p_cap: float) -> Path:
    ...
    for r in results:
        ...
        # The shared 10-90 anchor (tag "both") is written into each series so neither
        # curve loses its overlapping point. Solved once; reported in both.
