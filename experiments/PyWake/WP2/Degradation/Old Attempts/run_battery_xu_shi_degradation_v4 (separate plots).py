"""WP2 battery optimization + Shi degradation,  v4 (multi-year, calendar-corrected).
...
1. Calendar correction (Option 2)
   ...
   under the Xu model.  In v4, ft_calendar() from degradation_xu is added to
...
2. Multi-year degradation loop (MULTI_YEAR = True)
   Rather than running one year and projecting EoL analytically (v1), v3
...
3. Battery replacement at EOL_REPLACEMENT = 0.70 SoH
   When SoH drops below the 70% threshold within the multi-year loop,
...
--------------
  ...
  degradation_plots_multiyear.py  — multi-year trajectory + gradient figures (v4)
...
New in v4 vs v3:
  SoC continuity fix            — LP now locks soc_frac after year 1 so inter-year
                                   SoC% is consistent across degraded capacities
  Plot separation               — all multi-year plot functions moved to
                                   degradation_plots_multiyear.py; run script
                                   reduced from ~1720 to ~1205 lines
"""
...
# ── v2 additions ──────────────────────────────────────────────────────────────
MULTI_YEAR       = False     # False → single-year run identical to v1
...
def _build_run_label(ts: str, price_csv: Path, p_cap: float, e_cap: float, soc_min: float = 0.10, soc_max: float = 0.90) -> str:
    ...
    return f"{ts}_{dataset}_{bat}_{soc_tag}_v2"
