"""WP2 battery optimization + Shi degradation — v5.6 (single DoD window, wp2_econ).

PURPOSE
-------
Single-window, 20-year simulation with battery replacement.  No parameter sweep.
Use this to re-run the reference 10-90% SoC window case (or any other fixed window)
...
v5.4  — EUR-native econ via wp2_econ, proper per-year discounting, dual-basis NPV
          (battery-only marginal, arbitrage, total plant), symmetric efficiency,
          separate initial vs replacement capex (245 vs 72 EUR/kWh), deg_model
...
--------------------------
1. EUR-native throughout.  eur_to_usd conversion removed.  SHIPP price input is
   EUR directly (the kernel accepts EUR — the USD convention was a holdover).

2. Symmetric efficiency.  eff_in = eff_out = sqrt(rte_ac) = 0.9367, NOT
   eff_in=1.0 / eff_out=rte_ac.  Uses eta_symmetric() from wp2_econ.

3. Proper per-year discounting.  Revenue is tracked per year and discounted
   individually via wp2_econ.discount_weights (SHIPP year-1..N-1 convention).
   The old approach (single-year LP NPV minus PV of replacements) is replaced by
   the full per-year assembly from v5.4.

4. Split capex.  Initial capex uses e_cost / p_cost (245 / 86 EUR/kWh,kW from
   YAML).  Replacement capex uses repl_e / repl_p (72 / 96 EUR/kWh,kW from YAML).
   This matches the Danish Technology Catalogue rationale in WP2_Battery.yaml.

5. Dual NPV basis.  Both battery-only (marginal = total plant − wind-only) and
   total-plant NPV are reported, plus arbitrage and Plan B proxy, via revenue_annual.

6. deg_model toggle.  Set DEG_MODEL = "xu" to run the full Xu model comparison
   (doubles runtime); default is "shi" (fitted Phi + Xu calendar).

7. Single-year baseline.  The single-year SHIPP sizing solve still runs for the
   EFC, dispatch, and single-year degradation report, but NPV reporting is
...
- SHIPP annuity convention: n_year-1 revenue years (19 for N_YEARS=20). See
  wp2_econ.py HORIZON FLIP note.
"""
...
# Degradation model: "shi" (fitted Phi + Xu calendar, fast) or "xu" (full Xu, ~2x runtime)
DEG_MODEL = "shi"
...
def _build_run_label(
    ...
    return f"{ts}_{dataset}_{bat}_{soc_tag}_{FILE_TAG}"
...
def _build_shipp_components(
    ...
    - eff_in = eff_out = eta = sqrt(rte_ac) via eta_symmetric() instead of
      eff_in=1.0 / eff_out=rte_ac.
    - Separate repl_e / repl_p read from YAML for replacement cost and
      degradation valuation (72 / 96 EUR/kWh, kW).
    """
    bat = setup["battery"]
...
def _run_multiyear(
    ...
    print(f"Multi-year loop: {N_YEARS} yr | tile: {n} h ({period_days:.1f} d) | "
          ...
          f"model={deg_model} | replacement SoH<{EOL_REPLACEMENT*100:.0f}%")
    print(f"{'─'*70}")
    ...
    for year in range(1, N_YEARS + 1):  # 20 SoH boundary points; revenue discounted over 19 yrs (weights slice below)
        ...
        # ── Per-year Castillo gradient (Shi path only) ────────────────────
        grad_yr = None
...
def main() -> None:
    ...
    print(f"Shi + Xu calendar | multi-year | replacement | EUR-native | per-year NPV")
    ...
    if SAVE_REPORT:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write(f"WP2 BATTERY — {FILE_TAG} | Shi + Xu calendar | multi-year NPV (EUR)\n")
            f.write("=" * 80 + "\n\n")
            ...
            for (yr, soh_p, fd_c, _n_rep), (fd_y, fd_cyc, fd_cal), grad, rev in zip(
                multiyear["soh_trajectory"], ann_fd, ann_grad, ann_bat
            ):
