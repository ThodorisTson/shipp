...
Implements the per-timestep subgradient of the rainflow degradation cost, following the algorithm of:

  Shi, Y. et al., "A Convex Cycle-based Degradation Model for Battery Energy Storage Planning and Operation," ACC 2018.

This module sits on top of degradation_xu.py. It does NOT replace the Xu et al. physical model — that remains the validated ground truth for all degradation reporting. 
This module provides the gradient signal that connects the SHIPP dispatch LP to the outer sizing loop.
...
------------------------------------------------------
Xu's LMO S_δ(δ) is the validated physical ground truth for degradation reporting (fd, SoH, EoL curves). However it is only convex above
δ ≈ 0.1437 — Shi Theorem 1 requires global convexity of Φ.

A SoC-window floor does NOT fix this. SoC constraints bound the battery *state*, not *cycle depth*. Even with soc_min = 0.40, a small arbitrage
...
Solution (methodological decision, not a bug fix):
  ...
  - Shi Φ (phi_shi_prime_with_stress, polynomial k3·δ^k4):
        ...
        Fitted to Xu S_δ over [0.15, 0.80], k3 = 3.2418e-05, k4 = 1.1785 > 1 → globally convex.
...
from degradation_xu import (
    ...
    phi_shi_prime,                # depth-only Phi'(delta): the gradient path
    phi_shi_prime_with_stress,    # Phi'(delta)*S_sigma*S_T: comparison only
)
...
def build_half_cycle_map(
    ...
    Implements the T_{v_i} / T_{w_j} sets from Shi et al. (2018) Eqs. 14-15, which partition the time horizon into charging and discharging
    half-cycles. This partition is required to evaluate the per-timestep subgradient of the rainflow degradation cost.

    Strategy — loop over cycles, not timesteps
    ------------------------------------------
    A yearly simulation has ~8760 timesteps but typically only ~100-400 rainflow cycles. By iterating over cycles and using numpy slice
    assignment (array[i0:i1] = value), the inner work is a C-level vectorised write rather than a Python loop over each timestep.
    This makes the total Python-level iteration count ~200 instead of 8760, and the slice writes themselves are O(1) in Python overhead.

    Junction handling
    -----------------
    Some timesteps sit at the boundary between two cycles (e.g. point E' in Shi et al. Fig. 2). These are resolved by assigning the timestep
    to the deeper cycle — consistent with Shi et al.'s footnote on junction subgradient non-uniqueness. This is achieved by sorting
    cycles by DoD ascending before slicing, so deeper cycles overwrite shallower ones at shared indices.
    ...
    Direction is derived from np.diff(storage_e), which is fully vectorised with zero Python-level loop.

    Args:
        cycles:    Output of rainflow_cycle_counting() from degradation_xu.py. Each dict must contain:
                   'i_start', 'i_end', 'dod', 'soc_mean', 'count'.
        storage_e: Battery energy [MWh] time-series — the same array passed to rainflow_cycle_counting().
    ...
    # Widest span written first, narrowest last, so the innermost cycle owns each timestep. Sorting by DoD instead lets the deep, year-spanning residual
    # half-cycles of the annual envelope overwrite every cycle nested inside them. Ties are broken by depth, which is where Shi's junction footnote applies.
    sorted_cycles = sorted(
        enumerate(cycles),
        key=lambda x: (-abs(int(x[1]["i_end"]) - int(x[1]["i_start"])),
                       x[1]["dod"]),
    )
...
def compute_subgradient(
    ...
    # The gradient path uses the DEPTH-ONLY polynomial Phi(delta) = k3*delta^k4.
    # Two reasons, and both are structural rather than numerical:
    #
    #   1. Convexity. Shi Theorem 1 is stated for a stress function of cycle depth alone. Xu's S_delta is non-convex below delta ~0.1437, so the
    #      polynomial replaces it. Multiplying by S_sigma(sigma_i) would put the cost outside the theorem again, since sigma_i is a function of the SoC
    #      profile, not a constant.
    #
    #   2. Locality. The depth of a half-cycle is invariant to a uniform shift of the SoC trajectory; its mean is not. With an S_sigma factor, d(f)/d(c_t)
    #      picks up a contribution from EVERY cycle downstream of t, and on the annual dispatch that non-local term exceeds the local depth term by two
    #      orders of magnitude. Shi Eqs. 17-18 would then no longer be the derivative of the cost being computed.
    #
    # S_sigma and S_T stay where they belong: in the Xu reporting path and in the capacity gradient dDegCost/dE_cap, both of which carry no convexity claim.
    #
    # include_soc_stress=True restores the old behaviour. It is retained only to reproduce the comparison in the verification appendix, and must not be used
    # to produce gradients.
    if include_soc_stress:
        phi_prime = phi_shi_prime_with_stress(
            dod_at_t, soc_at_t, T_C, _fit.k3, _fit.k4, p
        )
    else:
        phi_prime = phi_shi_prime(dod_at_t, _fit.k3, _fit.k4)
