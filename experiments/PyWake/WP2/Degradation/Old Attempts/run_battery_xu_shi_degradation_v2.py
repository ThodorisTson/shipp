...
def _plot_gradient_analysis(
    ...
    fig.suptitle(
        f"Inner-Loop Gradient Analysis — Shi (2018)  |  "
        f"{N_YEARS} yr  |  replacement threshold: SoH < {EOL_REPLACEMENT*100:.0f}%",
        fontsize=11, y=1.01,
    )
    ...
    bar_colors   = ["#e74c3c" if yr in multiyear["replacement_years"]
                    else "#27ae60" if yr in {r + 1 for r in multiyear["replacement_years"]}
                    else "#4C72B0" for yr in valid_years]
    ...
    ax.set_title("Inner-Loop Gradient  dDeg/dDoD\n(sensitivity of degradation cost to dispatch)")
    ax.set_xlim(0.5, N_YEARS + 0.5)
    ax.set_xticks(range(2, N_YEARS + 1, 2))
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.25, axis="y")

    # ── Top-right: dDeg/dDoD vs SoH scatter ──────────────────────────────
    ax = axes[0, 1]
    sc = ax.scatter(v_soh, v_dDeg, c=v_years, cmap="plasma",
                    s=60, zorder=3, edgecolors="white", linewidths=0.5)
    for yr, soh, dd in zip(v_years, v_soh, v_dDeg):
        ax.annotate(str(yr), (soh, dd), fontsize=6.5,
                    xytext=(3, 3), textcoords="offset points", alpha=0.85)
    plt.colorbar(sc, ax=ax, label="Project year")
    ax.set_xlabel("State of Health [%]")
    ax.set_ylabel("dDeg/dDoD  [USD / MWh]")
    ax.set_title("dDeg/dDoD vs SoH\n(how gradient evolves as battery ages)")
    ...
    ax.set_title("Mean Absolute Subgradient per Year\n"
                "(reflects DoD distribution via Φ'(δ) — varies with LP dispatch)")
    ax.set_xlim(0.5, N_YEARS + 0.5)
    ax.set_xticks(range(2, N_YEARS + 1, 2))
    ax.grid(True, alpha=0.25, axis="y")

    # ── Bottom-right: cycle_coverage ──────────────────────────────────────
    ax = axes[1, 1]
    valid_cov_yrs = [yr for yr, cc in zip(years, cycle_cov) if cc is not None]
    valid_cov     = [cc for cc in cycle_cov if cc is not None]
    ax.bar(valid_cov_yrs, [cc * 100 for cc in valid_cov],
           width=0.7, color="#16a085", alpha=0.80)
    for yr in multiyear["replacement_years"]:
        ax.axvline(yr + 0.5, linestyle="-.", linewidth=1.4, color="red", alpha=0.8)
    ax.axhline(100, linestyle="--", linewidth=1.0, color="#2c3e50", alpha=0.5)
    ax.set_xlabel("Project year")
    ax.set_ylabel("Cycle coverage [%]")
    ax.set_title("Rainflow Cycle Coverage per Year\n"
                 "(fraction of timesteps attributed to a cycle)")
    ax.set_xlim(0.5, N_YEARS + 0.5)
    ax.set_xticks(range(2, N_YEARS + 1, 2))
    ax.set_ylim(0, 105)
    ax.grid(True, alpha=0.25, axis="y")
...
def _plot_subgradient_timeseries(
    ...
    fig.suptitle(
        f"Subgradient Time Series — Year 1 vs Year {panel_years[1]} (EoL)  |  "
        f"Shi (2018) inner-loop gradient",
        fontsize=11, y=1.01,
    )
    ...
    for ax, yr, idx, col in zip(axes, panel_years, panel_idxs, colours):
        ...
        ax.plot(t, subgrad, linewidth=0.6, color=col, alpha=0.85,
                label=f"subgrad_combined  (yr {yr}, SoH={soh_p:.1f}%)")
...
# Effective capacity: E_nominal × (SoH/100), reset at replacement
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 9))
    fig.suptitle(
        f"Multi-year Degradation — Shi (2018) + Xu calendar  |  "
        f"{N_YEARS} yr  |  replacement threshold: SoH < {EOL_REPLACEMENT*100:.0f}%",
        fontsize=11, y=1.01,
    )
    ...
    ax.set_title("SoH Trajectory\n(capacity fade feedback each year)")
    # Place legend in lower-left where SoH line hasn't reached yet
    ...
    # Annotate the acceleration explicitly — dark box in upper-left, away from bars
    if len(ann_fd_total) >= 2:
        pct_acc = 100.0 * (ann_fd_total[-1] - ann_fd_total[0]) / max(ann_fd_total[0], 1e-9)
        ax.annotate(
            f"+{pct_acc:.1f}% acceleration over {N_YEARS} yr\n"
            f"(capacity fade → deeper cycles → higher fd)",
            xy=(0.03, 0.97), xycoords="axes fraction",
            fontsize=7.5, color="#1a252f", ha="left", va="top",
    ...
    ax.set_title("Annual Degradation Rate\n"
                 "\"Corrected\" = Shi cycle fd + Xu calendar fd")
    ax.set_xlim(0.5, N_YEARS + 0.5)
    ...
    # For post-replacement years the reference is 100% (fresh battery),
    # not the previous year's degraded SoH.
    ...
    ax.set_title("Year-over-Year SoH Loss Rate\n"
                 "(increasing = degradation accelerating with capacity fade)")
    ax.set_xlim(0.5, N_YEARS + 0.5)
    ax.set_xticks(range(2, N_YEARS + 1, 2)) #even numbering 2,4,....,20
    ...
    ax.set_title(
        f"fd Decomposition: Cycle vs Calendar\n"
        f"Mean split: cycle {mean_cyc_pct:.0f}%  /  calendar {mean_cal_pct:.0f}%"
    )
    ax.set_xlim(0.5, N_YEARS + 0.5)
    ax.set_xticks(range(2, N_YEARS + 1, 2)) #even numbering 2,4,....,20
