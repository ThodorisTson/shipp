...
# ═════════════════════════════════════════════════════════════════════════════
# FIGURE 1 — dual-axis overlay, no shading
# ═════════════════════════════════════════════════════════════════════════════
fig1, ax1 = plt.subplots(figsize=(13, 5.5))
fig1.patch.set_facecolor('white')
ax1.set_facecolor('#f7f9fc')
ax1.grid(True, which='major', color='white', lw=1.0)
ax1.grid(True, which='minor', color='white', lw=0.4)

ax2 = ax1.twinx()

ax2.fill_between(t, pwr, alpha=0.15, color=WIND_COL, zorder=1)
ax2.plot(t, pwr, color=WIND_COL, lw=1.2, zorder=2)
...
ax1.fill_between(t, prc, alpha=0.15, color=PRICE_COL, zorder=1)
ax1.plot(t, prc, color=PRICE_COL, lw=1.2, zorder=3)
...
price_patch = mpatches.Patch(color=PRICE_COL, alpha=0.8,
                              label='DK1 price (EUR MWh⁻¹)')
wind_patch  = mpatches.Patch(color=WIND_COL,  alpha=0.8,
                              label='Wind farm power (MW)')
ax1.legend(handles=[price_patch, wind_patch], loc='upper left',
           fontsize=10, framealpha=0.9, edgecolor='#ccc')

plt.tight_layout()
fig1.savefig('/mnt/user-data/outputs/jan2022_overlay_clean.pdf', dpi=300,
             bbox_inches='tight', facecolor='white')
fig1.savefig('/mnt/user-data/outputs/jan2022_overlay_clean.png', dpi=200,
             bbox_inches='tight', facecolor='white')

# ═════════════════════════════════════════════════════════════════════════════
# FIGURE 2 — normalized single axis, no shading
# ═════════════════════════════════════════════════════════════════════════════
cf        = pwr / rated_farm                                      # capacity factor [0,1]
prc_norm  = (prc - prc.min()) / (prc.max() - prc.min())          # normalized price [0,1]

fig2, ax  = plt.subplots(figsize=(13, 5.5))
fig2.patch.set_facecolor('white')
ax.set_facecolor('#f7f9fc')
ax.grid(True, which='major', color='white', lw=1.0)
ax.grid(True, which='minor', color='white', lw=0.4)

ax.fill_between(t, cf,       alpha=0.15, color=WIND_COL,  zorder=1)
ax.fill_between(t, prc_norm, alpha=0.15, color=PRICE_COL, zorder=1)
ax.plot(t, cf,       color=WIND_COL,  lw=1.4, zorder=2)
ax.plot(t, prc_norm, color=PRICE_COL, lw=1.4, zorder=3)

ax.set_ylim(0, 1.08)
ax.set_ylabel('Normalized value (–)', fontsize=12)
style_xaxis(ax)
ax.tick_params(axis='y', labelsize=10)
ax.set_title('WP2 Site – Price and Wind Power, January 2022',
             fontsize=16, fontweight='bold', color='#0d1b2a', pad=10)

cf_patch  = mpatches.Patch(color=WIND_COL,  alpha=0.8, label='Wind capacity factor  (P / P$_{rated}$)')
pn_patch  = mpatches.Patch(color=PRICE_COL, alpha=0.8,
                            label='Normalized price  $(p - p_{min}) / (p_{max} - p_{min})$')
ax.legend(handles=[cf_patch, pn_patch], loc='upper left',
          fontsize=10, framealpha=0.9, edgecolor='#ccc')

plt.tight_layout()
fig2.savefig('/mnt/user-data/outputs/jan2022_normalized.pdf', dpi=300,
             bbox_inches='tight', facecolor='white')
fig2.savefig('/mnt/user-data/outputs/jan2022_normalized.png', dpi=200,
             bbox_inches='tight', facecolor='white')

import shutil, os
shutil.copy(__file__, '/mnt/user-data/outputs/plot_jan_v3.py')
print("Done.")
