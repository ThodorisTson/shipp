...
Strategy per memory note: two SEPARATE figures, never pooled.
  ...
  4. NPV vs P at fixed E=475 MWh (Xu grid optimum)                  → Fig sweep-D
...
# CONFIGURATION
# ═══════════════════════════════════════════════════════════════════════════
COARSE_TAG   = "172953"    # substring uniquely identifying the coarse CSV
ZOOM_TAG     = "203525"    # substring uniquely identifying the zoom CSV
SCENARIO     = "xu"        # primary degraded model for heatmaps ("xu" or "shi")
FIXED_P_MW   = 175         # fixed P for the NPV-vs-E line plot
FIXED_E_MWH  = 475         # fixed E for the NPV-vs-P line plot (Xu grid opt from zoom)
EUR_TO_MEUR  = 1e-6

# Zoom region boundaries — drawn as dashed box on the full heatmap
ZOOM_E_MIN, ZOOM_E_MAX = 300, 600    # MWh
ZOOM_P_MIN, ZOOM_P_MAX =  75, 200    # MW

# Quadratic-fit domain (zoom only)
QUAD_E_RANGE = (300, 600)
QUAD_P_RANGE = ( 75, 200)
...
def plot_full_heatmap(df: pd.DataFrame, scenario: str,
    ...
    ax.add_patch(rect)
    ax.text(ZOOM_P_MIN + 2, ZOOM_E_MAX + 15, "refined region",
            fontsize=FS_ANNOT, color=TUDELFT["navy"], style="italic")
...
def plot_zoom_heatmap(df: pd.DataFrame, scenario: str,
    ...
    cf = ax.tricontourf(tri, m[col].values, levels=16, cmap=CMAP_NPV)
    ax.tricontour(tri, m[col].values, levels=16,
                  colors="k", linewidths=0.20, alpha=0.35)
    cb = fig.colorbar(cf, ax=ax, shrink=0.88)
    cb.set_label("Lifetime NPV  (MEUR)")
